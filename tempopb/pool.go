...
//
// The upstream Prometheus buffer-pool miss metrics have been dropped from this
// fork to avoid a dependency on prometheus/client_golang; the TraceQL parser
// only needs the pooling behaviour, not the miss counters.
type Pool struct {
	buckets   []sync.Pool
	bktSize   int
	minBucket int
